...
- Persist application state by mounting a host directory into `/state` inside the container, or mount a directory to the container XDG config location for WebUI sessions.
...
| `SESSION_SECRET_FILE`          | No               | Path to a file containing the session secret (minimum 32 bytes, hex-encoded or plain text). Example: `/state/session_secret`. The file must be readable by the container.                                                                                                                                                                                                                                                                                                                                     |
...
| `UA_WEBUI_TRUSTED_PROXY_COUNT` | No               | Number of trusted reverse-proxy hops in front of the WebUI. Leave unset or `0` for direct access. Set to `1` for one trusted proxy. Never enable this merely because the WebUI is exposed to a LAN.                                                                                                                                                                                                                                                                                                           |
...
- When running inside a container the WebUI prefers the per-user XDG config directory for storing `session_secret` and `webui_auth.json`. By default that will be `/root/.config/upload-assistant` inside the container. Alternatively, set `SESSION_SECRET_FILE` below the persistent `/state` mount (for example `/state/session_secret`).
...
Mount one host directory for all application state (recommended). On the first WebUI start, the app creates `/state/data/config.py` from the bundled example if one is not already present. The directory does **not** need to exist on the host — Docker creates it and the entrypoint fixes ownership automatically:

- `/host/path/Upload-Assistant:/state:rw`

> **Tip:** Mount the whole state directory at `/state`, not a single `config.py` file. The configuration then lives at `/state/data/config.py`.

Optional mounts (recommended for persistence and predictable behavior):

- Temporary release files, caches, cookies and `config.py` live under the same `/state` mount.
...
services:
  upload-assistant:
    image: ghcr.io/darkvader-cell/upload-assistant-wastaken:latest
    ...
    environment:
      - PUID=1000
      - PGID=1000
      - UA_BROWSE_ROOTS=/data/torrents,/state/tmp
      # - SESSION_SECRET_FILE=/state/session_secret
      # - IN_DOCKER=1
      # - UA_WEBUI_CORS_ORIGINS=https://your-ui-host
      # - UA_WEBUI_TRUSTED_PROXY_COUNT=1
    ...
    volumes:
      ...
      - /path/to/appdata/Upload-Assistant:/state:rw
...
- **Mounting application state** (recommended): mount the whole state root at `/state`. On the first WebUI start, the app automatically creates `data/config.py` below it. You can then edit it via the WebUI config editor.
...
- Set the appdata path to a stable folder, e.g. `/mnt/user/appdata/Upload-Assistant`, and bind it into `/state` inside the container.
...
services:
  upload-assistant:
    ...
    ports:
      - "5000:5000"
    volumes:
      - /mnt/user/appdata/Upload-Assistant:/state:rw
...
The entrypoint script automatically fixes ownership of `/state`, `session_secret` (when `SESSION_SECRET_FILE` points to a directory), and `/root/.config/upload-assistant` (webui-auth mount) at startup when `PUID`/`PGID` are set. No manual `chown` is needed for typical setups.

If you need to adjust permissions manually (e.g. for bind mounts with special requirements):

```bash
# For standard systems (UID 1000)
sudo chown -R 1000:1000 /host/path/Upload-Assistant
