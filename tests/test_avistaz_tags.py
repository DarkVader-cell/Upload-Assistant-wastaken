...
TRACKERS = ((AvistaZ, "AVISTAZ", "3773", "943"), (CinemaZ, "CINEMAZ", "1594", "938"), (PrivateHD, "PRIVATEHD", "1448", "415"))
...
def test_personal_release_tag_is_added_without_keywords(tracker_class, tracker_name, personal_tag, _internal_tag):
    tags = asyncio.run(get_tags(tracker_class({"TRACKERS": {tracker_name: {}}}), SimpleNamespace(keywords=[], personalrelease=True)))
...
def test_internal_release_tag_is_added_without_keywords(tracker_class, tracker_name, _personal_tag, internal_tag):
    tags = asyncio.run(get_tags(tracker_class({"TRACKERS": {tracker_name: {"internal": True}}}), SimpleNamespace(keywords=[], personalrelease=False)))
