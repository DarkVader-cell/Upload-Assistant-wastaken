...
async def get_service(
    ...
    for key, value in services.items():
        ...
        if len(filename_alias.split()) > 3:
            continue
        # Compact service names, not filename fields, so titles and tags stay separate.
        for alias in (filename_alias, filename_alias.replace(" ", "")):
            if (
                len(alias) > longest_match
                and (" " + alias + " ") in video_name
                and key not in title_guess_title
                and key not in title_guess_episode_title
                and alias not in title_guess_title
                and alias not in title_guess_episode_title
            ):
                service = value
