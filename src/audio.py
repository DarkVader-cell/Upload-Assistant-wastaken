...
async def _get_audio_v2(
        ...
        if isinstance(additional, str) and "atmos" in additional.lower():
            ...
            try:
                ...
                if base_dir and folder_id:
                    mi_path = Path(base_dir) / "tmp" / str(folder_id) / "MediaInfo.json"
    ...
    if bdinfo is None or bd_mi is not None:  # Rips or BD with mediainfo
        ...
        meta.non_disc_has_pcm_audio_tracks = meta.type != "DISC" and any(track.get("Format") == "PCM" for track in audio_tracks)
        ...
        logger.debug(f"DEBUG: Channels: {channels}, Channel Layout: {channel_layout}, Additional: {additional}, Format: {audio_format}")
        chan = determine_channel_count(channels, channel_layout, additional, audio_format)
    ...
    format_settings = (
        "EX"
        if format_settings == "EX"
        and (chan == "5.1" or (codec == "DD+" and chan == "7.1"))
        and not source_key.startswith("WEB")
        else ""
    )
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            if is_pair_hd_lossy and has_meaningful_properties and track_one_properties == track_two_properties:
                ...
                if not warned_once:
                    warned_once = True
                    logger.info(f"[bold red]DTS audio track #{lossy_idx} appears to be a lossy duplicate of DTS-HD MA track #{hd_idx}.[/bold red]")
